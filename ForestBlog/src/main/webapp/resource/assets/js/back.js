...
function approveComment(id) {
    $.ajax({
        async: false,
        type: "POST",
        url:getPath()+'/admin/comment/approve/'+id,
        contentType : "application/x-www-form-urlencoded; charset=utf-8",
        dataType: "text",
        complete:function () {
            window.location.reload();
...
function hideComment(id) {
    $.ajax({
        async: false,
        type: "POST",
        url:getPath()+'/admin/comment/hide/'+id,
        contentType : "application/x-www-form-urlencoded; charset=utf-8",
        dataType: "text",
        complete:function () {
            window.location.reload();
...
function insertDraft() {
    if($("#articleContent").val!=""&&$("#articleTitle").val()!="") {
        $.ajax({
            async: false,
            type: "POST",
            url:getPath()+'/admin/article/insert',
            contentType : "application/x-www-form-urlencoded; charset=utf-8",
            dataType: "text",
            complete:function () {
                window.location.reload();
...
function deleteArticle(id) {
    if(confirmDelete()==true){
        $.ajax({
            async: false,
            type: "POST",
            url:getPath()+'/admin/article/delete/'+id,
            contentType : "application/x-www-form-urlencoded; charset=utf-8",
            dataType: "text",
            complete:function () {
                window.location.reload();
...
function queryArticle() {
    //提交form
    $("#articleForm").attr("action", getPath()+"/admin/article/search");
    $("#articleForm").submit();
}
...
function confirmDeleteArticleBatch() {
    if(confirmDelete()==true){
        ...
        $.ajax({
            async: false,
            type: "POST",
            url:getPath()+'/admin/article/deleteBatch',
            data:{ids:text},
            contentType : "application/x-www-form-urlencoded; charset=utf-8",
            dataType: "text",
            complete:function () {
...
function checkUserName() {
    ...
    $.ajax({
        ...
        type: "POST",
        url: getPath()+'/admin/user/checkUserName',
        contentType: "application/x-www-form-urlencoded; charset=utf-8",
...
function checkUserEmail() {
    ...
    $.ajax({
        async: false,//同步，待请求完毕后再执行后面的代码
        type: "POST",
        url: getPath()+'/admin/user/checkUserEmail',
        contentType: "application/x-www-form-urlencoded; charset=utf-8",
