...
(function($){
    ...
    function write(){
        var content = "<div class=\"ds-dialog\" id='PAY_"+id+"'>"
            ...
            + "       <img  class=\"qrcode-img qrCode_0\" id=\"qrCode_0\" src="+getPath()+"/img/shang/weixinpay.jpg"+"  />"
            + "      </div>"
            + "      <p class=\"qrcode-tip\">打赏</p>"
            + "     </div>"
            + "     <div class=\"qrcode-img qrCode_1\" id=\"qrCode_1\">"
            + "      <div class=\"qrcode-border box-size\" style=\"border: 9.02px solid rgb(102, 153, 204\">"
            + "       <img  class=\"qrcode-img qrCode_1\" id=\"qrCode_1\"  src="+getPath()+"/img/shang/qqpay.jpg"+"  />"
            + "      </div>"
            + "      <p class=\"qrcode-tip\">打赏</p>"
            + "     </div>"
            + "     <div class=\"qrcode-img qrCode_2\" id=\"qrCode_2\">"
            + "      <div class=\"qrcode-border box-size\" style=\"border: 9.02px solid rgb(235, 95, 1\">"
            + "       <img  class=\"qrcode-img qrCode_2\" id=\"qrCode_2\"  src="+getPath()+"/img/shang/alipay.jpg"+"  />"
            + "      </div>"
            + "      <p class=\"qrcode-tip\">打赏</p>"
            + "     </div>"
            + "     </div>"
...
function deleteComment(id) {
    if(confirmDelete()==true){
        $.ajax({
            async: false,
            type: "POST",
            url:getPath()+'/admin/comment/delete/'+id,
            contentType : "application/x-www-form-urlencoded; charset=utf-8",
            dataType: "text",
            complete:function () {
                window.location.reload();
...
function increaseViewCount() {
    if ($.cookie("viewId") != articleId || $.cookie("viewId") == null) {
        $.ajax({
            async: false,
            type: "POST",
            url: getPath()+"/article/addView/"+articleId,
...
function increaseLikeCount() {
    if ($.cookie("likeId") != articleId || $.cookie("likeId") == null) {
        $.ajax({
            async: false,
            type: "POST",
            url: getPath()+"/article/addLike/"+articleId,
            contentType : "application/x-www-form-urlencoded",
            success: function (data) {
                $(".count").html(data);
                $.cookie(
...
//评论cookie 和 弹出层
$(".reply-btn").click(function (a) {
    var url = this.href;
    var s = url.match(/(\d+)[^\d]*$/);
    var commentId = s[0];
    $.cookie(
        "commentId",
        commentId,//需要cookie写入的业务
        {
            "path": "/", //cookie的默认属性
        }
    );
    ...
    layer.open({
        ...
        content: [getPath() + '/reply-comment', 'yes'],
    });
})
